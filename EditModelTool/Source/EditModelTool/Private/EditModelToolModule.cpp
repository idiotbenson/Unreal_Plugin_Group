...
namespace
{
    ...
    bool AssignedMaterialChainMatchesSubstring(UMaterialInterface* RootMat, const FString& QueryTrimmed)
    {
        ...
        while (Walk && Depth++ < 64)
        {
            ...
            if (Walk->GetName().Contains(QueryTrimmed, ESearchCase::IgnoreCase))
            {
                return true;
            }
    ...
    bool StaticMeshComponentUsesAssignedMaterialSubstring(UStaticMeshComponent* SMC, const FString& QueryTrimmed)
    {
        if (!SMC || QueryTrimmed.IsEmpty())
        {
            return false;
        ...
        for (int32 SlotIdx = 0; SlotIdx < NumSlots; ++SlotIdx)
        {
            ...
            if (AssignedMaterialChainMatchesSubstring(Mat, QueryTrimmed))
            {
                return true;
            }
        }
    ...
    bool ActorUsesAssignedMaterialSubstringOnMeshComponents(AActor* Actor, const FString& QueryTrimmed)
    {
        if (!Actor || QueryTrimmed.IsEmpty())
        {
            return false;
        ...
        for (UStaticMeshComponent* SMC : Components)
        {
            if (StaticMeshComponentUsesAssignedMaterialSubstring(SMC, QueryTrimmed))
            {
                return true;
            }
        }
...
void FEditModelToolModule::OpenRenameDialog()
{
    struct FDialogWidgetState
    {
        ...
        TSharedPtr<SEditableTextBox> MaterialSlotQueryBox;
        TSharedPtr<SEditableTextBox> MaterialAssignedQueryBox;
    ...
    DialogWindow = SNew(SWindow)
        ...
        .ClientSize(FVector2D(300.f, 680.f))
    ...
    const auto BuildMaterialSlotSection = [this, DialogState]() -> TSharedRef<SWidget>
    {
        return SNew(SVerticalBox)
            + SVerticalBox::Slot()
            .AutoHeight()
            ...
            [
                SNew(STextBlock)
                .AutoWrapText(true)
                .Text(LOCTEXT("MaterialSlotQueryHint", "Material slot name on StaticMesh (substring, case-insensitive, whole level):"))
            ]
            + SVerticalBox::Slot()
            .AutoHeight()
            .Padding(0.f, 0.f, 0.f, 6.f)
            [
                ...
                .Padding(0.f, 0.f, 6.f, 0.f)
                [
                    SAssignNew(DialogState->MaterialSlotQueryBox, SEditableTextBox)
                    .HintText(LOCTEXT("MaterialSlotQueryPlaceholder", "e.g. Glass_Panel"))
                    .OnTextCommitted_Lambda([this, DialogState](const FText&, ETextCommit::Type CommitType)
                    {
                        if (CommitType != ETextCommit::OnEnter)
                        {
                            return;
                        }
                        const FString Q = GetTrimmedText(DialogState->MaterialSlotQueryBox);
                        if (Q.IsEmpty())
                        {
                            FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("MaterialSlotQueryEmpty", "Please enter text to match material slot names."));
                            return;
                        }
                        SelectActorsWithMaterialSlotNameInLevel(Q);
                    })
                ]
                + SHorizontalBox::Slot()
                .AutoWidth()
                [
                    SNew(SButton)
                    .Text(LOCTEXT("SelectByMaterialSlotInLevelButton", "Select: material slot (level)"))
                    .ToolTipText(LOCTEXT(
                        "SelectByMaterialSlotInLevelTooltip",
                        "Scans the current level: selects any actor that has at least one UStaticMeshComponent whose StaticMesh asset defines a material slot whose name contains the typed text (substring, case-insensitive). Session filter applies."))
                    .OnClicked_Lambda([this, DialogState]()
                    {
                        const FString Q = GetTrimmedText(DialogState->MaterialSlotQueryBox);
                        if (Q.IsEmpty())
            ...
            [
                SNew(SButton)
                .Text(LOCTEXT("SelectNaniteInLevelButton", "Select: Nanite meshes (whole level)"))
                .ToolTipText(LOCTEXT(
                    "SelectNaniteInLevelTooltip",
                    "Scans all actors in the current level and selects actors that have at least one StaticMeshComponent whose StaticMesh asset has Nanite enabled."))
                .OnClicked_Lambda([this]()
                {
                    SelectActorsUsingNaniteInLevel();
                    return FReply::Handled();
                })
            ]
            + SVerticalBox::Slot()
            ...
            [
                ...
                .AutoWrapText(true)
                .Text(LOCTEXT(
                    "MaterialAssignedQueryHint",
                    "Assigned material on meshes (per-slot GetMaterial): substring on material object name only (case-insensitive; path ignored). Same name under different folders matches the same query. Whole level:"))
            ]
            + SVerticalBox::Slot()
            .AutoHeight()
            .Padding(0.f, 0.f, 0.f, 6.f)
            [
                ...
                .Padding(0.f, 0.f, 6.f, 0.f)
                [
                    SAssignNew(DialogState->MaterialAssignedQueryBox, SEditableTextBox)
                    .HintText(LOCTEXT("MaterialAssignedQueryPlaceholder", "e.g. MI_Glass"))
                    .OnTextCommitted_Lambda([this, DialogState](const FText&, ETextCommit::Type CommitType)
                    {
                        if (CommitType != ETextCommit::OnEnter)
                        {
                        ...
                        const FString Q = GetTrimmedText(DialogState->MaterialAssignedQueryBox);
                        if (Q.IsEmpty())
                        {
                            FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("MaterialAssignedQueryEmpty", "Please enter text to match assigned materials."));
                            return;
                        }
                        SelectActorsWithAssignedMaterialSubstringInLevel(Q);
                    })
                ]
                + SHorizontalBox::Slot()
                .AutoWidth()
                [
                    SNew(SButton)
                    .Text(LOCTEXT("SelectByAssignedMaterialInLevelButton", "Select: assigned material (level)"))
                    .ToolTipText(LOCTEXT(
                        "SelectByAssignedMaterialInLevelTooltip",
                        "Uses each StaticMesh component's slot materials (GetMaterial). Matches if any slot's assigned material—or its Material Instance parent chain—has an object Name (not path) containing the typed text, so duplicates under different folders still match the same substring. Includes overrides and extra slots."))
                    .OnClicked_Lambda([this, DialogState]()
                    {
                        const FString Q = GetTrimmedText(DialogState->MaterialAssignedQueryBox);
                        if (Q.IsEmpty())
...
void FEditModelToolModule::SelectActorsWithAssignedMaterialSubstringInLevel(const FString& Substring)
{
    const FString Q = Substring.TrimStartAndEnd();
    if (Q.IsEmpty())
    {
    ...
    for (TActorIterator<AActor> It(World); It; ++It)
    {
        ...
        if (!ActorUsesAssignedMaterialSubstringOnMeshComponents(Actor, Q))
        {
            continue;
        }
        Matches.Add(Actor);
    ...
    FMessageDialog::Open(
        ...
        FText::Format(
            LOCTEXT(
                ...
                "Assigned material scan complete.\nQuery: \"{0}\" (substring in material object name only, instance chain; path ignored)\nMatched actor count: {1}."),
            FText::FromString(Q),
            FText::AsNumber(Matches.Num())));
}
