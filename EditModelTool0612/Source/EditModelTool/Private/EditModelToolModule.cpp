    ...
    *DialogState->SearchModeOptionIndices[2] = NoneOptionIndex;
    ...
    const auto BuildActionSection = [this, RequestCloseDialog, DialogState]() -> TSharedRef<SWidget>
    {
        return SNew(SVerticalBox)
            ...
            [
                SNew(SButton)
                .Text(LOCTEXT("KeepStaticMeshActorsInSelectionButton", "Keep Static Mesh Actors In Selection"))
                .ToolTipText(LOCTEXT(
                    "KeepStaticMeshActorsInSelectionTooltip",
                    "From the current outliner or viewport selection, deselect plain Actor types and actors without static meshes. Keeps StaticMeshActor and other mesh-host actor types. Hidden attach parents are ignored."))
                .OnClicked_Lambda([this]()
                {
                    KeepStaticMeshActorsInCurrentSelection();
                    return FReply::Handled();
                })
            ];
    };
...
namespace
{
    static bool IsStaticMeshHostActorForSelectionFilter(const AActor* Actor)
    {
        if (!Actor)
        {
            return false;
        ...
        return Actor->GetClass() != AActor::StaticClass();
    }
}

void FEditModelToolModule::KeepStaticMeshActorsInCurrentSelection()
{
    TArray<AActor*> RawSelectedActors;
    EditModelToolSelectionUtils::GatherSelectedActors(RawSelectedActors, /*bRequireEditorVisible*/ false);
    if (RawSelectedActors.Num() == 0)
    {
        FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("KeepStaticMeshNoSelection", "No actors selected in outliner. Select actors or Static Mesh components in the World Outliner (or viewport), then try again."));
        return;
    }

    TArray<AActor*> StaticMeshActors;
    StaticMeshActors.Reserve(RawSelectedActors.Num());
    for (AActor* SelectedActor : RawSelectedActors)
    {
        if (IsStaticMeshHostActorForSelectionFilter(SelectedActor))
        {
            StaticMeshActors.Add(SelectedActor);
        }
    }

    EditModelToolSelectionUtils::ApplyActorSelection(StaticMeshActors, /*bRequireEditorVisible*/ false);
    ...
    if (StaticMeshActors.Num() == 0)
    {
        FMessageDialog::Open(
            EAppMsgType::Ok,
            LOCTEXT(
                "KeepStaticMeshActorsInSelectionNone",
                "Current selection has no static-mesh hosts. Plain empty Actor types are skipped. Hidden attach parents are ignored."));
    }
}
