			...
			.Next([SharedComps, DeltaZWorld, ProgressText, StageText](TArray<FIndexPair> Cands)
			{
				AsyncTask(
					...
					[SharedComps, Cands, DeltaZWorld, ProgressText, StageText]()
					{
						...
						int32 Applied = 0;
						TArray<AActor*> LiftedActorsUnique;
						{
							...
							for (const FValidatedPair& Pr : ExclusivePairs)
							{
								...
								LiftedActorsUnique.AddUnique(ToMove);
								++Applied;
							}
						}

						if (Applied > 0 && GEditor)
						{
							GEditor->SelectNone(true, true, false);
							for (AActor* Lifted : LiftedActorsUnique)
							{
								if (Lifted && IsValid(Lifted))
								{
									GEditor->SelectActor(Lifted, true, false, true);
								}
							}
						...
						FMessageDialog::Open(
							...
							FText::Format(
								LOCTEXT(
									...
									"Shape match: same StaticMesh, same DerivedDataKey, or identical LOD0 vertex/triangle counts with strong geometry hash (quantized verts + canonical faces). Each part pairs with at most one partner. Lifted actors are now selected."),
